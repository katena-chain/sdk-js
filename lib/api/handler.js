...
class Handler {
  ...
  async retrieveLastCertificateTx(fqId) {
    const apiResponse = await this._apiClient.get(sprintf('%s/%s/%s', Handler.CERTIFICATES_PATH, fqId, Handler.LAST_PATH))
  ...
  async retrieveCertificateTxs(fqId, page, txPerPage) {
    const apiResponse = await this._apiClient.get(sprintf('%s/%s', Handler.CERTIFICATES_PATH, fqId), getPaginationQueryParams(page, txPerPage))
    if (apiResponse.getStatusCode() === HttpStatus.OK) {
  ...
  async retrieveLastSecretTx(fqId) {
    const apiResponse = await this._apiClient.get(sprintf('%s/%s/%s', Handler.SECRETS_PATH, fqId, Handler.LAST_PATH))
    if (apiResponse.getStatusCode() === HttpStatus.OK) {
      return deserialize(TxResult, JSON.parse(apiResponse.getBody()))
    } else {
      throw ApiError.fromJSON(apiResponse.getBody())
  ...
  async retrieveSecretTxs(fqId, page, txPerPage) {
    const apiResponse = await this._apiClient.get(sprintf('%s/%s', Handler.SECRETS_PATH, fqId), getPaginationQueryParams(page, txPerPage))
    if (apiResponse.getStatusCode() === HttpStatus.OK) {
      return deserialize(TxResults, JSON.parse(apiResponse.getBody()))
    } else {
      throw ApiError.fromJSON(apiResponse.getBody())
  ...
  async retrieveLastKeyTx(fqId) {
    const apiResponse = await this._apiClient.get(sprintf('%s/%s/%s', Handler.KEYS_PATH, fqId, Handler.LAST_PATH))
    if (apiResponse.getStatusCode() === HttpStatus.OK) {
      return deserialize(TxResult, JSON.parse(apiResponse.getBody()))
    } else {
      throw ApiError.fromJSON(apiResponse.getBody())
  ...
  async retrieveKeyTxs(fqId, page, txPerPage) {
    const apiResponse = await this._apiClient.get(sprintf('%s/%s', Handler.KEYS_PATH, fqId), getPaginationQueryParams(page, txPerPage))
    if (apiResponse.getStatusCode() === HttpStatus.OK) {
      return deserialize(TxResults, JSON.parse(apiResponse.getBody()))
    } else {
      throw ApiError.fromJSON(apiResponse.getBody())
